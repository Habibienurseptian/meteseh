    ...
    <div class="bg-white p-6 sm:p-8 rounded-xl shadow-lg">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            ...
            <div class="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 w-full sm:w-auto">
                ...
                {{-- Add Booking Button --}}
                <a href="{{ route('staf.bookings.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-300 ease-in-out w-full sm:w-auto text-center">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap hidden sm:table-cell">Nama Ketua</th>
                        ...
                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap hidden xl:table-cell">Waktu Kedatangan</th>
                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap hidden md:table-cell">Status</th>
                        ...
                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap hidden md:table-cell">Maktab</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($recentBookings as $booking)
                        <tr>
                            ...
                            <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-800 hidden sm:table-cell">{{ $booking->leader_name ?? 'N/A' }}</td>
                            <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-800 hidden lg:table-cell">{{ $booking->number_of_pilgrims ?? 'N/A' }}</td>
                            <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-800 hidden xl:table-cell">{{ \Carbon\Carbon::parse($booking->arrival_time)->format('Y-m-d H:i A') ?? 'N/A' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="11" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">Tidak ada booking terbaru yang tersedia.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
